...
# DESCRIPTION:  Automating the JOBS EVSE excel codebase from VBA in Excel to Python 
...
for i in range(len(inputs)):
    ...
    if inputs["yes/no to include onsite storage costs"][i] == "Yes" or inputs["yes/no to include onsite storage costs"][i] == "yes": 
        if inputs["Charger Power"][i] == 50 or inputs["Charger Power"][i] == 150 or inputs["Charger Power"][i] == 350:
            cost2008 = inputs["Number of chargers per station"][i] * inputs["Number of stations"][i] * inputs["Charger Power"][i] * 400
        else:
            cost2008 = 0
...
for i in range(len(inputs)):
    # Find tier employment per charger:
    cost2008 = 0
    if inputs["Charger Power"][i] <= 19.2:
        cost2008 = ((1000/2) * 0.84)
    else:
        cost2008 = 3000  
...
for i in range(len(inputs)):
    ...
    if inputs["yes/no to include meters"][i] == "Yes" or inputs["yes/no to include meters"][i] == "yes":
        if (inputs["Charger Power"][i] <= 6.6) or (6.6 < inputs["Charger Power"][i] and inputs["Charger Power"][i] <= 19.2):
            cost2008 = 0
        else:
            cost2008 = 2000 * 0.84
...
for i in range(len(inputs)):
    # Find tier employment per charger:
    cost2008 = 0
    if (inputs["Charger Power"][i] <= 6.6) or (6.6 < inputs["Charger Power"][i] and inputs["Charger Power"][i] <= 19.2) or (inputs["Charger Power"][i] == 50):
        cost2008 = ((1000 + 100)/2) * 0.84
    else:
        cost2008 = 2000 * 0.84
...
def physical_components_total(i):
    ...
    # Charger:
    if inputs["Charger Power"][i] <= 6.6:
        total_cost += ((200+530)/2)
    elif 6.6 < inputs["Charger Power"][i] and inputs["Charger Power"][i] <= 19.2:
        total_cost += ((900+4900)/2) 
    elif inputs["Charger Power"][i] == 50:
        total_cost += (27900) 
    elif inputs["Charger Power"][i] == 150:
        total_cost += (87800) 
...
#     all_totals_df.to_excel(writer, sheet_name = "EVSE Component Totals")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     civil_all_totals_df.to_excel(writer, sheet_name = "Civil Construction Employment:")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     elec_all_totals_df.to_excel(writer, sheet_name = "Electrical Construction Employment:")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     retail_all_totals_df.to_excel(writer, sheet_name = "Retail Sector Employment:")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     ad_all_totals_df.to_excel(writer, sheet_name = "Advertising Sector Employment:")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     data_all_totals_df.to_excel(writer, sheet_name = "Data and Networking Sector Employment:")
...
# Final Part: Adding DataFrame as a new excel sheet in User-Input Excel sheet:
# with pd.ExcelWriter(input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:  
#     warr_all_totals_df.to_excel(writer, sheet_name = "Warranty, Maintenance, Administrative Costs, & Access Fees Sector Employment")
